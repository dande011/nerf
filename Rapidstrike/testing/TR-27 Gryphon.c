...
void loop() {
	...
	Check(Burst, false);
	Check(Auto, false);
	if(Gun.Safty){Blinkables.RightDP = true;}else{Blinkables.RightDP = false;};
	if(!Gun.Loaded && !Blinkables.Dashes && !Blinkables.RightDP){Blinkables.Dashes = true; Blinkables.Display = true;}
	else if(Gun.Loaded && Blinkables.Dashes){Blinkables.Dashes = false; Blinkables.Display = false;};
	Blinker(BlinkTimer, BlinkStep, Blinkables);
	Sleep();
	CheckLid();
	if(Gun.PlungerStep == 0){CheckMode(Mode, ModeTimer);};
	CheckAmmo();
	switch(Mode){
	case 1:
		//3 BURST
